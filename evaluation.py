...
def evaluate_model(model_name: str, test_dir: str,
    ...
    """Evaluate a speaker verification model on a test set.
    ...
    for i in range(0, len(test_files), batch_size):
        ...
        for file1 in batch_files:
            for file2 in test_files[i+1:]:
                ...
                spk2 = os.path.basename(file2).split('_')[0]
                is_same_speaker = spk1 == spk2
                
                # Get similarity score
                vector1 = exctract_vector_embedding(file1, model_name, model, feature_extractor)
                vector2 = exctract_vector_embedding(file2, model_name, model, feature_extractor)
                
                similarity_score = np.dot(vector1, vector2.T)[0][0]  # Cosine similarity
...
def plot_roc_curve(model_name: str, test_dir: str,
                   model: object = None, feature_extractor: object = None, 
    ...
    for i in range(0, len(test_files), batch_size):
        ...
        for file1 in batch_files:
            for file2 in test_files[i+1:]:
                ...
                similarity_score = np.dot(vector1, vector2.T)[0][0]
                ...
                # Log progress
                if len(y_true) % 100 == 0:
                    logging.info(f"Processed {len(y_true)}/{total_pairs} pairs")
